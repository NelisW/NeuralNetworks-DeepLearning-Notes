...
There are two steps to getting an exploding gradient. First, we choose all the weights in the network to be large, say $w_1$=w2=w3=w4=100
...
*  Identity neuron: Consider a neuron with a single input, x, a corresponding weight, $w_1$, a bias b, and a weight $w_2$ on the output. Show that by choosing the weights and bias appropriately, we can ensure $w_2$?(w1x+b)?x for x?[0,1]. Such a neuron can thus be used as a kind of identity neuron, that is, a neuron whose output is the same (up to rescaling by a weight factor) as its input. Hint: It helps to rewrite x=1/2+?, to assume $w_1$ is small, and to use a Taylor series expansion in $w_1$?
* . 
