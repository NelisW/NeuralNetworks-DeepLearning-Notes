...
The original version of this chapter (see the URI above) has interactive graphics where the reader can change values and observe the effect. The \LaTeX{} version does not have the interactivity. You are advised to visit the web page.  Alternatively, the repository that contains the \LaTeX{} source code has a Jupyter notebook with interactive plotting widgets, see \lstinline{code/chapter4-graphics.ipynb}.
